(async function () {
  ...
  var utilities = require('./support/utilities'); // Polyfill buffers.
  ...
  var images = [await fs_readFilePromise('test/fixture/1.png'), await fs_readFilePromise('test/fixture/2.png'), await fs_readFilePromise('test/fixture/32.png'), await fs_readFilePromise('test/fixture/1293.jpg'), await fs_readFilePromise('test/fixture/1294.png')];
  ...
  describe('invalid images', function () {
    ...
    it('should error out with "Incorrect bKGD chunk index value"', function (done) {
      blend([images[2]], {
        ...
        assert.ok(data.length > 600 && data.length < 1200, 'reencoding bogus image yields implausible size'); // Check that we don't reencode the error.
    ...
    it('should report a bogus Huffman table definition', function (done) {
      blend([images[2], images[3]], function (err, data, warnings) {
        ...
        assert.equal(err.message, 'JPEG Reader: libjpeg could not read image: Bogus Huffman table definition'); // Test working state after error.
    ...
    it('should error out on blending an invalid JPEG image', async function (done) {
      var buffer = new Buffer(32);
      buffer.fill(0);
      buffer[0] = 0xFF;
      buffer[1] = 0xD8;
      blend([buffer, await fs_readFilePromise('test/fixture/2.png')], function (err, data) {
        assert.ok(err);
        assert.ok(err.message);
        done();
      });
    });
    it('should error out on blending a malformed JPEG image', async function (done) {
      blend([await fs_readFilePromise('test/fixture/1d.jpg'), await fs_readFilePromise('test/fixture/2.png')], function (err, data) {
        assert.ok(err);
        assert.ok(err.message);
        done();
      });
