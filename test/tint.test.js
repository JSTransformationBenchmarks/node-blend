...
describe('tinting', async function () {
  (await fs_readdirPromise('./test/tinted')).filter(function (file) {
    return path.extname(file) === '.png' && file.indexOf('.result.') == -1;
    ...
    it(testName, async function (done) {
      var options = {
        width: 256,
        height: 256,
        quality: 256,
      ...
      tint([{
        buffer: await fs_readFilePromise('./test/source/' + name + '.png'),
        tint: new_o
...
var TINTS = ["0x1;0x1;0x1;0x1", // no change
".5x1;1x1;0x1;0x1", // teal
"0x1;0x1;0x1;0x.5", // half alpha
".1x1;.3x1;0x.9;0x1", // sepia
"0;54;1;0;1"];
describe('tinting combinations on varied images', async function () {
  (await fs_readdirPromise('./test/fixture/tinting')).filter(function (file) {
    return (path.extname(file) === '.png' || path.extname(file) === '.jpeg') && file.indexOf('.result.') == -1;
  }).forEach(function (file) {
    TINTS.forEach(function (tinter) {
      ...
      it(file + '-' + tinter.toString(), async function (done) {
        var buf = await fs_readFilePromise('./test/fixture/tinting/' + file);
        var tint_obj;
